...
public class FileSystemResource extends AbstractResource implements WritableResource {
    ...
    public FileSystemResource(File file) {
        AssertTools.notNull(file, "File must not be null");
        this.path = PathTools.cleanPath(file.getPath());
        this.file = file;
    ...
    public FileSystemResource(Path filePath) {
        AssertTools.notNull(filePath, "Path must not be null");
        this.path = PathTools.cleanPath(filePath.toString());
        this.file = null;
    ...
    public FileSystemResource(FileSystem fileSystem, String path) {
        AssertTools.notNull(fileSystem, "FileSystem must not be null");
        AssertTools.notNull(path, "Path must not be null");
        this.path = PathTools.cleanPath(path);
        this.file = null;
    ...
    public String getContentAsString(Charset charset) throws IOException {
        try {
            return new String(Files.readAllBytes(this.filePath), charset);
        } catch (NoSuchFileException ex) {
            throw new FileNotFoundException(ex.getMessage());
        }
    }
    ...
    public URI getURI() throws IOException {
        if (this.file != null) {
            ...
            if (ResourceTools.URL_PROTOCOL_FILE.equals(scheme)) {
                try {
