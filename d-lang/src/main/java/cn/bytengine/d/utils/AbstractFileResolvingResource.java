...
public abstract class AbstractFileResolvingResource extends AbstractResource {
    ...
    public boolean exists() {
        try {
            URL url = getURL();
            if (ResourceTools.isFileURL(url)) {
            ...
            if (ResourceTools.isFileURL(url)) {
                ...
                if (con instanceof HttpURLConnection) {
                    ...
                    if (code == HttpURLConnection.HTTP_BAD_METHOD) {
                        con = url.openConnection();
                        customizeConnection(con);
                        boolean isHttpUrlConnection = (con instanceof HttpURLConnection);
    ...
    public boolean isFile() {
        try {
            URL url = getURL();
            if (url.getProtocol().startsWith(ResourceTools.URL_PROTOCOL_VFS)) {
                return VfsResourceDelegate.getResource(url).isFile();
            }
            return ResourceTools.URL_PROTOCOL_FILE.equals(url.getProtocol());
        } catch (IOException ex) {
            return false;
        }
    }
    ...
    public File getFile() throws IOException {
        ...
        return ResourceTools.getFile(url, getDescription());
    }
    ...
    protected File getFileForLastModifiedCheck() throws IOException {
        URL url = getURL();
        if (ResourceTools.isJarURL(url)) {
            URL actualUrl = ResourceTools.extractArchiveURL(url);
            if (actualUrl.getProtocol().startsWith(ResourceTools.URL_PROTOCOL_VFS)) {
                return VfsResourceDelegate.getResource(actualUrl).getFile();
            }
            return ResourceTools.getFile(actualUrl, "Jar URL");
        } else {
            return getFile();
        }
    }
    ...
    protected boolean isFile(URI uri) {
        try {
            if (uri.getScheme().startsWith(ResourceTools.URL_PROTOCOL_VFS)) {
                return VfsResourceDelegate.getResource(uri).isFile();
            }
            return ResourceTools.URL_PROTOCOL_FILE.equals(uri.getScheme());
        } catch (IOException ex) {
            return false;
        }
    }
     ...
     * @see ResourceTools#getFile(java.net.URI, String)
     */
    protected File getFile(URI uri) throws IOException {
        if (uri.getScheme().startsWith(ResourceTools.URL_PROTOCOL_VFS)) {
            return VfsResourceDelegate.getResource(uri).getFile();
        }
        return ResourceTools.getFile(uri, getDescription());
    }
    ...
    public long contentLength() throws IOException {
        ...
        if (ResourceTools.isFileURL(url)) {
            ...
            if (length <= 0 && isHttpURLConnection) {
                HttpURLConnection httpCon = (HttpURLConnection) con;
                if (httpCon.getResponseCode() == HttpURLConnection.HTTP_BAD_METHOD) {
                    con = url.openConnection();
                    customizeConnection(con);
    ...
    public long lastModified() throws IOException {
        ...
        if (ResourceTools.isFileURL(url) || ResourceTools.isJarURL(url)) {
            // Proceed with file system resolution
            fileCheck = true;
            try {
     ...
     * <p>Calls {@link ResourceTools#useCachesIfNecessary(URLConnection)} and
    ...
    protected void customizeConnection(URLConnection con) throws IOException {
        ResourceTools.useCachesIfNecessary(con);
        if (con instanceof HttpURLConnection) {
            HttpURLConnection httpCon = (HttpURLConnection) con;
    ...
    private static class VfsResourceDelegate {
        ...
        public static Resource getResource(URL url) throws IOException {
            return new VfsResource(VfsTools.getRoot(url));
        }

        public static Resource getResource(URI uri) throws IOException {
            return new VfsResource(VfsTools.getRoot(uri));
        }
    }
