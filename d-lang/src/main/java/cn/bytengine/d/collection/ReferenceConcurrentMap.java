...
public class ReferenceConcurrentMap<K, V> implements ConcurrentMap<K, V>, Iterable<Map.Entry<K, V>>, Serializable {
    ...
    private Reference<K> ofKey(K key, ReferenceQueue<? super K> queue) {
        switch (keyType) {
            case WEAK:
                return new ReferenceConcurrentMap.WeakKey<>(key, queue);
            case SOFT:
                return new ReferenceConcurrentMap.SoftKey<>(key, queue);
        }
    ...
    private static class WeakKey<K> extends WeakReference<K> {
        ...
        public boolean equals(Object other) {
            if (other == this) {
                return true;
            } else if (other instanceof ReferenceConcurrentMap.WeakKey) {
                return ObjectTools.equals(((ReferenceConcurrentMap.WeakKey<?>) other).get(), get());
            }
            return false;
        }
    }
    ...
    private static class SoftKey<K> extends SoftReference<K> {
        ...
        public boolean equals(Object other) {
            if (other == this) {
                return true;
            } else if (other instanceof ReferenceConcurrentMap.SoftKey) {
                return ObjectTools.equals(((ReferenceConcurrentMap.SoftKey<?>) other).get(), get());
            }
            return false;
        }
    }
